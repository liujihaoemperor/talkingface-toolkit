...
from talkingface.trainer.model_sum_trainer import *
...
def run_talkingface(
    ...
    trainer = cota_train(trainer)
    trainer = syn_train(trainer)
    trainer = gta_extract(trainer)

    # model training
    if config['train']:
        trainer.fit(train_data_loader, val_data_loader, saved=saved,
