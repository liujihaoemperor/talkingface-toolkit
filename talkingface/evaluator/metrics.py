...
class LSE(SyncMetric):
    ...
    def metric_info(self, videofile):
        ...
        if os.path.exists(os.path.join(self.config['temp_dir'], self.config['lse_reference_dir'])):
            rmtree(os.path.join(
                self.config['temp_dir'], self.config['lse_reference_dir']))

        os.makedirs(os.path.join(
            self.config['temp_dir'], self.config['lse_reference_dir']))

        command = ("ffmpeg -loglevel error -y -i %s -threads 1 -f image2 %s" % (videofile,
                   os.path.join(self.config['temp_dir'], self.config['lse_reference_dir'], '%06d.jpg')))
        output = subprocess.call(command, shell=True, stdout=None)

        command = ("ffmpeg -loglevel error -y -i %s -async 1 -ac 1 -vn -acodec pcm_s16le -ar 16000 %s" %
                   (videofile, os.path.join(self.config['temp_dir'], self.config['lse_reference_dir'], 'audio.wav')))
        output = subprocess.call(command, shell=True, stdout=None)

        images = []

        flist = glob.glob(os.path.join(
            self.config['temp_dir'], self.config['lse_reference_dir'], '*.jpg'))
        flist.sort()
        ...
        sample_rate, audio = wavfile.read(os.path.join(
            self.config['temp_dir'], self.config['lse_reference_dir'], 'audio.wav'))
        ...
        for i in range(0, lastframe, self.config['evaluate_batch_size']):
            ...
            im_batch = [imtv[:, :, vframe:vframe+5, :, :]
                        for vframe in range(i, min(lastframe, i+self.config['evaluate_batch_size']))]
