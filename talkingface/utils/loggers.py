...
class TacotronLogger(TensorBoardLogger):
    ...
    def log_figures(self, mel_pred, mel_postnet, mel_target, alignment, step):
        ...
        alignment_plot = plot_alignments(
            alignment[0], alignment[rand_idx], transpose=True)
        self.experiment.add_figure(
            'alignment_fixed_random', alignment_plot, step)

        spectrogram_plot = plot_spectrograms(
            mel_pred[rand_idx], mel_postnet[rand_idx], mel_target[rand_idx])
...
class SynthesizerLogger(TensorBoardLogger):
    ...
    def log_figures(self, mel_source, mel_s_s, mel_s_t, alignment, residual, step):
        ...
        alignment_plot = plot_alignments(
            alignment[0], alignment[rand_idx], transpose=True)
        self.experiment.add_figure(
            'pretrained_alignment_fixed_random', alignment_plot, step)
