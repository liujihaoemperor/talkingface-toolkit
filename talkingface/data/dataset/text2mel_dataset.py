...
class TextMelDataset(Dataset):
    def __init__(self, hp, data_dir, metadata_path, train=True, norm=False, use_f0s=False):
        ...
        self.speaker_dict = {speaker: idx for idx,
                             speaker in enumerate(hp.data.speakers)}
    ...
    def get_text(self, text):
        if self.cmudict is not None:
            text = ' '.join([self.get_arpabet(word)
                            for word in text.split(' ')])
        text_norm = torch.IntTensor(
            self.lang.text_to_sequence(text, self.hp.data.text_cleaners))
        return text_norm
