...
class Cotatron(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.symbols = Language(
            hp.data.lang, hp.data.text_cleaners).get_symbols()
    ...
    def forward(self, text, mel_target, speakers, input_lengths, output_lengths, max_input_len,
        ...
        text_encoding = self.encoder(
            text, input_lengths)  # [B, T, chn.encoder]
        speaker_emb = self.speaker(
            mel_target, output_lengths)  # [B, chn.speaker]
        speaker_emb_rep = speaker_emb.unsqueeze(
            1).expand(-1, text_encoding.size(1), -1)  # [B, T, chn.speaker]
        # [B, T, (chn.encoder + chn.speaker)]
    ...
    def inference(self, text, mel_target):
        ...
        speaker_emb_rep = speaker_emb.unsqueeze(
            1).expand(-1, text_encoding.size(1), -1)
        decoder_input = torch.cat((text_encoding, speaker_emb_rep), dim=2)
    ...
    def training_step(self, batch, batch_idx):
        ...
        speaker_emb, mel_pred, mel_postnet, alignment = \
            self(text, mel_target, speakers, input_lengths,
                 output_lengths, max_input_len)
        speaker_prob = self.classifier(speaker_emb)
        classifier_loss = F.nll_loss(speaker_prob, speakers)

        loss_rec = F.mse_loss(mel_pred, mel_target) + \
            F.mse_loss(mel_postnet, mel_target)
        self.logger.log_loss(loss_rec, mode='train',
                             step=self.global_step, name='reconstruction')
        self.logger.log_loss(classifier_loss, mode='train',
                             step=self.global_step, name='classifier')

        if self.use_attn_loss:
            attention_loss = self.attn_loss(
                alignment, input_lengths, output_lengths, self.global_step)
            self.logger.log_loss(attention_loss, mode='train',
                                 step=self.global_step, name='attention')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.is_val_first:  # plot alignment, character embedding
            ...
            self.logger.log_figures(
                mel_pred, mel_postnet, mel_target, alignment, self.global_step)
            self.logger.log_embedding(
                self.symbols, self.encoder.embedding.weight, self.global_step)
    ...
    def validation_epoch_end(self, outputs):
        ...
        classifier_loss = torch.stack(
            [x['classifier_loss'] for x in outputs]).mean()
        self.logger.log_loss(loss_rec, mode='val',
                             step=self.global_step, name='reconstruction')
        self.logger.log_loss(classifier_loss, mode='val',
                             step=self.global_step, name='classifier')
        self.is_val_first = True
    ...
    def lr_lambda(self, step):
        progress = (step - self.hp.train.decay.start) / \
            (self.hp.train.decay.end - self.hp.train.decay.start)
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, optimizer_closure=None, on_tpu=False, using_native_amp=False, using_lbfgs=False):
        ...
        self.logger.log_learning_rate(
            lr_scale * self.hp.train.adam.lr, self.global_step)
